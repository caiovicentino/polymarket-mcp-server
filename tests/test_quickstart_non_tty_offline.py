...
Re-delivery r3 (post-revert): `pty` is POSIX-only and the 1st delivery
imported it bare, so windows-latest CI crashed at COLLECTION with
ModuleNotFoundError, failing the WHOLE suite (not just the pty tests) —
that is why squash 81cefb7 was auto-reverted (T-0224 revertlog,
2026-09-17T18:58:41). The module-level pytest.importorskip("pty") below
(AGENTS.md item 147 cross-platform rule) turns that collection crash into
a module-level SKIP on non-POSIX hosts (7 skipped, rc=0); on macOS/Linux
the module resolves and all 7 tests still run. Windows never executes the
test bodies, so the QUICKSTART.read_text() below is unreachable there
(the site-1 ✗ glyph in quickstart.sh stays pre-existing ASCII-out-of-scope).
"""
import os
import select
import signal
...
pty = pytest.importorskip("pty")  # POSIX-only; module skips on Windows (r3, item 147)
