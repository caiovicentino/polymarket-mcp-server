...
def mock_polymarket_client():
    ...
    client.get_balance = AsyncMock(return_value={"balance": "1000.50"})
...
class TestPositionTools:
    ...
    @pytest.mark.asyncio
    async def test_get_all_positions_basic(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
        ...
        result = await get_all_positions(mock_polymarket_client, mock_rate_limiter, mock_config)
    ...
    @pytest.mark.asyncio
    async def test_get_all_positions_with_filters(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
        """Test positions with filters"""
        httpx_mock.add_response(url="https://data-api.polymarket.com/positions", json=[])
    ...
    @pytest.mark.asyncio
    async def test_get_position_details(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
        ...
        result = await get_position_details(
            mock_polymarket_client, mock_rate_limiter, mock_config, market_id="market_123"
        )
    ...
    async def test_get_portfolio_value(
        ...
        result = await get_portfolio_value(
            mock_polymarket_client, mock_rate_limiter, mock_config, include_breakdown=True
        )
    ...
    @pytest.mark.asyncio
    async def test_get_pnl_summary(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
        """Test P&L summary"""
        # Mock trades
        ...
        result = await get_pnl_summary(
            mock_polymarket_client, mock_rate_limiter, mock_config, timeframe="24h"
        )
...
class TestActivityTools:
    ...
    async def test_get_trade_history(
        ...
        result = await get_trade_history(
            mock_polymarket_client, mock_rate_limiter, mock_config, limit=50, side="BUY"
        )
    ...
    async def test_get_activity_log(
        ...
        result = await get_activity_log(
            mock_polymarket_client, mock_rate_limiter, mock_config, activity_type="trades"
        )
...
class TestAnalysisTools:
    ...
    @pytest.mark.asyncio
    async def test_suggest_portfolio_actions(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
        ...
        result = await suggest_portfolio_actions(
            mock_polymarket_client, mock_rate_limiter, mock_config, goal="balanced", max_actions=5
        )
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_no_positions_found(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
    ...
    @pytest.mark.asyncio
    async def test_invalid_market_id(
        self, mock_polymarket_client, mock_rate_limiter, mock_config, httpx_mock
    ):
        ...
        result = await get_position_details(
            mock_polymarket_client, mock_rate_limiter, mock_config, market_id="invalid_market"
        )
    ...
    async def test_api_error_handling(
        ...
        httpx_mock.add_response(url="https://data-api.polymarket.com/positions", status_code=500)
...
class TestRateLimiting:
    ...
    async def test_rate_limiter_called(
        ...
        assert any(call[0][0] == EndpointCategory.DATA_API for call in calls)
